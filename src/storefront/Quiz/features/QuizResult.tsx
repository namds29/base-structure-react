...
const QuizResult = () => {
  ...
  const correctAnswers = listQuestion && listQuestion.map((question) => question.correct_answer);
  ...
  return (
    <main>
      ...
      <div className={`w-1/2 mt-6 p-4 ml-64 text-center ${(matchingCount === 0 || matchingCount === 1) && "bg-red-300"}
      ${(matchingCount === 2 || matchingCount === 3) && "bg-yellow-400"}
      ${(matchingCount === 4 || matchingCount === 5) && "bg-green-400"}
    `} >Your score is {matchingCount} out of 5</div>
      <Link to="/">
        <button
